...
def reorder_using_information_gain(X, y):
    ...
    feature_information_gains.sort(key=lambda x: x[1], reverse=Tgitrue)
...
def test_stop_criterion(X, y, classifier, feature_selection, feature_reorder,
    ...
    steps_number = 0
    steps_number2 = 0
    steps_number_values = np.array([])
    steps_number_values2 = np.array([])
    ...
    for train, test in folds:
        ...
        if criterion_optimizer2 is not None:
            classifier.stop_criterion2, steps_number2  = criterion_optimizer2(X_train_selected,
                                                            y_train)
            steps_number_values2 = np.append(steps_number_values2,
                                               steps_number2)
        ...
        if criterion_optimizer is not None:
            classifier.stop_criterion, steps_number = criterion_optimizer(X_train_selected,
                                                            y_train)
            steps_number_values = np.append(steps_number_values,
                                             steps_number)
    ...
    if save_data is not None:
        save_data.add_data(stop_criterion_values, 0, steps_number_values, 0, stop_criterion_values)
        save_data.add_data(stop_criterion_values2, 0, steps_number_values2, 0, stop_criterion_values2)
    return 0, 0
...
def optimize_sequential_classification_stop_criterion(X, y,
    ...
    for stop_criterion in criterion_range:
        ...
        if mean_accuracy + offset < target_accuracy:
            ...
            last_value = stop_criterion
    if mode == "test":
        return last_value, steps_number
    else:
        return last_value